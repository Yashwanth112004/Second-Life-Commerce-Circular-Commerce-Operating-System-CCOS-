        ...
        <button onClick={() => setOpen(!open)} className="flex w-full items-center gap-3 py-2.5 text-left">
          ...
          {s.actionable > 0 && (
            <span className="hidden shrink-0 items-center gap-2 sm:flex">
              <span className="pill bg-leaf-500/20 text-leaf-400">${s.total_value_recovery_usd}</span>
              <span className="pill bg-sky-500/15 text-sky-300">{s.total_carbon_opportunity_kg} kg CO₂</span>
            </span>
          )}
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="pb-4">
                <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                  <MiniStat label="Value recovery" value={`$${s.total_value_recovery_usd}`} />
                  <MiniStat label="Carbon opportunity" value={`${s.total_carbon_opportunity_kg} kg`} />
                  <MiniStat label="Green Credits" value={`+${s.total_gc_opportunity}`} />
                  <MiniStat label="Circular Score" value={`+${s.total_circular_score_opportunity}`} />
                </div>

                {d.recommendations.filter((r) => r.action !== "hold").length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {d.recommendations.filter((r) => r.action !== "hold").slice(0, 4).map((r) => (
                          ...
                          {expanded === r.order_id && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-2 border-t border-white/5 pt-2">
                              ...
                              <div className="mt-2 text-xs text-slate-500">
                                Twin: ${r.twin.current_value} now → ${r.twin.forecast.m6} in 6mo · best window: {r.twin.best_resale_window} · CES {r.impact.ces_score}/100
                              </div>
                            </motion.div>
                          )}
                        </AnimatePresence>
                ...
                {listIt.isSuccess && (
                  <div className="mt-2 rounded-lg bg-leaf-500/10 p-2 text-xs text-leaf-300">
                    ✓ Agent listed "{listIt.data.listing.title}" at ${listIt.data.listing.price} with {listIt.data.buyer_matches.matches.length} buyer matches. <Link to="/agent" className="underline">View agent →</Link>
                  </div>
                )}
              </div>
            </motion.div>
